...
public partial class CustomProductController : BasePublicController
{
    ...
    public CustomProductController(CaptchaSettings captchaSettings,
    ...
    public virtual async Task<IActionResult> ProductDetails(int productId, int updatecartitemid = 0)
    {
        ...
        //model
        var model = await _customProductModelFactory.PrepareProductDetailsModelAsync(product, updatecartitem, false);
        //template
        var productTemplateViewPath = await _customProductModelFactory.PrepareProductTemplateViewPathAsync(product);

        return View("~/Plugins/NopValley.Misc/Views/Product/ProductTemplate.Simple.cshtml", model);
    }
