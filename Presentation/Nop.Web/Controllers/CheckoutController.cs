...
public partial class CheckoutController : BasePublicController
{
    ...
    protected virtual async Task SavePickupOptionAsync(PickupPoint pickupPoint)
    {
        ...
        await _genericAttributeService.SaveAttributeAsync(customer, SystemCustomerAttributeNames.BillToSameAddress, false, store.Id);
    ...
    protected virtual async Task<JsonResult> OpcLoadStepAfterShippingMethod(IList<ShoppingCartItem> cart)
    {
       ...
       // var cart = await _shoppingCartService.GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, store.Id);
        ...
        var billingAddressModel = new CheckoutBillingAddressModel();
        await _checkoutModelFactory.PrepareBillingAddressModelAsync(billingAddressModel, cart, prePopulateNewAddressWithCustomerFields:true);
                
        //see if billing to ship is checked
        if (await _genericAttributeService.GetAttributeAsync<bool>(customer, SystemCustomerAttributeNames.BillToSameAddress, store.Id))
            return await OpcLoadStepAfterBillingAddress(cart);
    ...
    protected virtual async Task<JsonResult> OpcLoadStepAfterBillingAddress(IList<ShoppingCartItem> cart)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var store = await _storeContext.GetCurrentStoreAsync();
        ...
        //Check whether payment workflow is required
        //we ignore reward points during cart total calculation
        bool isPaymentWorkflowRequired = await _orderProcessingService.IsPaymentWorkflowRequiredAsync(cart, false);
        if (isPaymentWorkflowRequired)
        {
            //filter by country
    ...
    public virtual async Task<IActionResult> OpcSaveBilling(CheckoutBillingAddressModel model, IFormCollection form)
    {
        try
        {
            ...
            if (Request.Form["ShipToSameAddress"] == "true,false")//on
            {
                //var address = _workContext.CurrentCustomer.Addresses.FirstOrDefault(a => a.Id == _workContext.CurrentCustomer.ShippingAddress.Id);


                //var address = await _customerService.GetCustomerAddressAsync(customer.Id)
                ...
                var address = (await _customerService.GetAddressesByCustomerIdAsync(customer.Id))
                    .FirstOrDefault(a => a.Id == customer.ShippingAddress.Id);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (address == null)
                    {
                        ...
                        {
                            address.Country = await _countryService.GetCountryByIdAsync(address.CountryId.Value);
                        }
        ...
        try
        {
            ...
            if (_shippingSettings.AllowPickupInStore && !_orderSettings.DisplayPickupInStoreOnShippingMethodPage)
            {
                ...
                if (pickupInStore)
                {
                   ...
                   await _shoppingCartService.RemoveRecurringOrderProductFromCart(customer);
                    // update the cart in case recurring order product has been added or removed (required for next step)
            ...
            if (shippingAddressId > 0)
            {
                ...
                if (Request.Form["BillToSameAddress"] == "true,false")
                {
                    await _genericAttributeService.SaveAttributeAsync(customer, SystemCustomerAttributeNames.BillToSameAddress, true, store.Id);
                    customer.BillingAddress = address;
                }
                else
                {
            ...
            else
            {
                ...
                customer.ShippingAddress = address;
                if (Request.Form["BillToSameAddress"] == "false")
                {
                    await _genericAttributeService.SaveAttributeAsync(customer, SystemCustomerAttributeNames.BillToSameAddress, true, store.Id);
                    customer.BillingAddress = address;
                }
            ...
            // Recurring order stuff
            RecurringOrderContext cxt = new RecurringOrderContext();

            try
            {
                var model1 = new CheckoutShippingAddressModel();
                
                cxt.IsRecurring = model1.IsRecurringOrder;
                cxt.RecurringOrderPeriod = model1.RecurringOrderPeriod;
                cxt.Name = model.RecurringOrderName;

                if (model1.IsRecurringOrder)
                {
                    _shoppingCartService.AddRecurringOrderProductToCart(customer, store.Id);
                }
                else
                {
                    await _shoppingCartService.RemoveRecurringOrderProductFromCart(customer);
            ...
            catch
            {
                ...
                cxt.RecurringOrderPeriod = RecurringOrderPeriod.Every1Week;
            ...
            if (cxt.IsRecurring && cxt.RecurringOrderPeriod == 0)
            {
                throw new Exception("You must select a recurring order period");
            }

            
            //await _httpContextAccessor.HttpContext.Session.SetAsync<RecurringOrderContext>("RecurringOrderContext", cxt);

            await HttpContext.Session.SetAsync<RecurringOrderContext>("RecurringOrderContext", cxt);
    ...
    public virtual async Task<IActionResult> OpcConfirmOrder(bool captchaValid)
    {
        try
        {
            ...
            if (!isCaptchaSettingEnabled || (isCaptchaSettingEnabled && captchaValid))
            {
                ...
                //
                processPaymentRequest.CustomValues.Add("RecurringOrderContext",
                     await HttpContext.Session.GetAsync<RecurringOrderContext>("RecurringOrderContext")); 
