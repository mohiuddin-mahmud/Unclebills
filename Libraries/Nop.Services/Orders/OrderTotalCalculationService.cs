...
public partial class OrderTotalCalculationService : IOrderTotalCalculationService
{
    ...
    public virtual async Task<decimal?> CalculateRecurringOrderTotal(Order initialOrder, IList<ShoppingCartItem> cart,
        bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true, bool reflectStock = false, int? recurringOrderId = null)
    {
        IList<ShoppingCartItem> newCart = new List<ShoppingCartItem>();
        foreach (var item in cart)
        {
            newCart.Add(new ShoppingCartItem
            {
                Id = item.Id,
        ...
        if (reflectStock)
            newCart = RemoveOutOfStockItemsFromCart(newCart);
       ...
       // List<Discount> appliedDiscounts;
        ...
        var customer = newCart.FirstOrDefault().Customer;
        string paymentMethodSystemName = "Payments.AuthorizeNet";

        if (customer == null)
        {
            throw new Exception("customer should not be null");
        }
        ...
        var (orderSubTotalDiscountAmountBase, _, subTotalWithoutDiscountBase, subTotalWithDiscountBase, _) = await GetShoppingCartSubTotalAsync(cart, subTotalIncludingTax);

        //GetShoppingCartSubTotalAsync(newCart, false,
        //    out orderSubTotalDiscountAmount, out orderSubTotalAppliedDiscounts,
        //    out subTotalWithoutDiscountBase, out subTotalWithDiscountBase);



        // var (_, _, subTotalWithoutDiscount, subTotalWithDiscount, taxRates) = await GetShoppingCartSubTotalAsync(cart, _shippingSettings.FreeShippingOverXIncludingTax);

       // var (subTotalWithDiscountBase, _) = await _taxService.GetProductPriceAsync(product, subTotal);
        ...
        if (initialOrder.OrderShippingExclTax == initialOrder.OrderShippingInclTax)
        {
            var shippingOptions = _shippingService.GetShippingOptionsAsync(newCart, initialOrder.ShippingAddress, customer, "Shipping.CourierNow").Result;
            shoppingCartShipping = shippingOptions.ShippingOptions.FirstOrDefault()?.Rate;
        }
        else
        {
            throw new Exception("shipping rate mismatch");
        }    
        ...
        if (_shoppingCartSettings.RoundPricesDuringCalculation)
            resultTemp = await _priceCalculationService.RoundPriceAsync(resultTemp, await _workContext.GetWorkingCurrencyAsync());
        ...
        {
            if (!useRewardPoints.HasValue)
                useRewardPoints = await _genericAttributeService.GetAttributeAsync<bool>(customer, NopCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, _storeContext.GetCurrentStore().Id);
            if (useRewardPoints.Value)
            {
                int rewardPointsBalance = _rewardPointService.GetRewardPointsBalanceAsync(customer.Id, _storeContext.GetCurrentStore().Id).Result;
                if (CheckMinimumRewardPointsToUseRequirement(rewardPointsBalance))
                {
                    decimal rewardPointsBalanceAmount = await ConvertRewardPointsToAmountAsync(rewardPointsBalance);
                    if (orderTotal > decimal.Zero)
                    {
                        if (orderTotal > rewardPointsBalanceAmount)
                        {
        ...
        if (_shoppingCartSettings.RoundPricesDuringCalculation)            
            orderTotal = await _priceCalculationService.RoundPriceAsync(orderTotal, await _workContext.GetWorkingCurrencyAsync());

        return orderTotal;
    }

    private IList<ShoppingCartItem> RemoveOutOfStockItemsFromCart(IList<ShoppingCartItem> cart)
    {
        IList<ShoppingCartItem> revisedCart = new List<ShoppingCartItem>();

        foreach (var item in cart)
        {
            if (item.Product.ManageInventoryMethod == ManageInventoryMethod.ManageStock)
            {
                var qtyInStock = _productService.GetTotalStockQuantityAsync(item.Product).Result;

                if (qtyInStock > 0)
                {
                    if (qtyInStock >= item.Quantity)
                    {
                        revisedCart.Add(item);
                    }
                    else
                    {
                        item.Quantity = qtyInStock;
                        revisedCart.Add(item);
                    }
                }
            }
            else if (item.Product.ManageInventoryMethod == ManageInventoryMethod.DontManageStock)
                revisedCart.Add(item);
        }
    ...
    private void DoNothing(IList<ShoppingCartItem> cart)
    {
        foreach (var item in cart)
        {
            item.Quantity = 4;
