...
public delegate Task MyEventHandler(int recurringOrderId);
...
public partial class ShoppingCartService : IShoppingCartService
{
    ...
    public event MyEventHandler UpdateRecurringOrderSubscription;
    ...
    public virtual async Task<IList<string>> AddToCartAsync(Customer customer, Product product,
        ...
        int? recurringOrderId = null, bool recurringOrderInitialCreation = false)
    {
        ArgumentNullException.ThrowIfNull(customer);
    ...
    public async void AddRecurringOrderProductToCart(Customer customer, int storeId)
    {
        var product = _productService.GetRecurringOrderProduct();
        await AddToCartAsync(customer, product, ShoppingCartType.ShoppingCart, storeId);
        
    }

    public async Task RemoveRecurringOrderProductFromCart(Customer customer)
    {
        //var query = from e in _sciRepository.Table
        //            where e.Product.Sku == "RECURRINGORDER" && e.ShoppingCartTypeId == (int)ShoppingCartType.ShoppingCart && e.CustomerId == customer.Id
        //            select e;
        ...
        var cart = await GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, store.Id);



        IList<ShoppingCartItem> customCarts = new List<ShoppingCartItem>();

        foreach (var cartItem in cart)
        {
            var product = await _productService.GetProductByIdAsync(cartItem.ProductId);
            if(product.Sku == "RECURRINGORDER" && cartItem.ShoppingCartTypeId == (int)ShoppingCartType.ShoppingCart)
            {
                customCarts.Add(cartItem);
            }
        }


        var shoppingCartItems = customCarts.ToList();

        if (shoppingCartItems.Any())
        {
            // in case more than one is in there
