...
public partial class WorkflowMessageService : IWorkflowMessageService
{
    ...
    public virtual async Task<IList<int>> SendRecurringOrderStoreNotification(RecurringOrder recurringOrder, int languageId, List<ShoppingCartItem> items, Order initialOrder)
    {
        if (recurringOrder == null)
            throw new ArgumentNullException("recurringOrder");


        var store = await _storeService.GetStoreByIdAsync(initialOrder.StoreId) ?? await _storeContext.GetCurrentStoreAsync();
        languageId = await EnsureLanguageIsActiveAsync(languageId, store.Id);

        var messageTemplates = await GetActiveMessageTemplatesAsync(MessageTemplateSystemNames.RecurringOrderStoreNotification, store.Id);
        if (!messageTemplates.Any())
            return new List<int>();

        // customer
        var customer = await _customerService.GetCustomerByIdAsync(recurringOrder.CustomerId);
        ...
        return await messageTemplates.SelectAwait(async messageTemplate =>
        {
            ...
            await _messageTokenProvider.AddStoreTokensAsync(tokens, store, emailAccount, languageId);
            await _messageTokenProvider.AddRecurringOrderTokens(tokens, recurringOrder, items, initialOrder);
    ...
    public virtual async Task<IList<int>> SendRecurringOrderCustomerNotification(RecurringOrder recurringOrder, int languageId, List<ShoppingCartItem> items, Order initialOrder)
    {
        if (recurringOrder == null)
            throw new ArgumentNullException("recurringOrder");
        ...
        var messageTemplates = await GetActiveMessageTemplatesAsync(MessageTemplateSystemNames.RecurringOrderCustomerNotification, store.Id);
        if (!messageTemplates.Any())
            return new List<int>();
    ...
    public virtual async Task<IList<int>> SendRecurringOrderCancelledCustomerNotification(RecurringOrder recurringOrder, int languageId, List<ShoppingCartItem> items, Order initialOrder)
    {
        if (recurringOrder == null)
            throw new ArgumentNullException("recurringOrder");
        ...
        var messageTemplates = await GetActiveMessageTemplatesAsync(MessageTemplateSystemNames.RecurringOrderCancelledCustomerNotification, store.Id);
        if (!messageTemplates.Any())
            return new List<int>();      

        // customer
        var customer = _customerService.GetCustomerByIdAsync(recurringOrder.CustomerId).Result;
    ...
    public virtual async Task<IList<int>> SendRecurringOrderCancelledStoreNotification(RecurringOrder recurringOrder, int languageId, List<ShoppingCartItem> items, Order initialOrder)
    {
        if (recurringOrder == null)
            throw new ArgumentNullException("recurringOrder");
        ...
        var messageTemplates = await GetActiveMessageTemplatesAsync(MessageTemplateSystemNames.RecurringOrderCancelledStoreNotification, store.Id);
        if (!messageTemplates.Any())
            return new List<int>();
    ...
    public virtual async Task<IList<int>> SendFailedCertsNotification(string fileName, string filePath, int languageId)
    {
        ...
        string messageTemplateName = MessageTemplateSystemNames.FailedCertificateNotification;
        ...
        return await messageTemplates.SelectAwait(async messageTemplate =>
        {
            ...
            return await SendNotificationAsync(messageTemplate, emailAccount, languageId, tokens, toEmail, toName, filePath, fileName);
        }).ToListAsync();
    }
