...
public partial class MessageTokenProvider : IMessageTokenProvider
{
    ...
    public virtual async Task AddRecurringOrderTokens(IList<Token> tokens, RecurringOrder recurringOrder, List<ShoppingCartItem> items, Order initialOrder)
    {
        tokens.Add(new Token("RecurringOrder.Name", recurringOrder.Name));
        tokens.Add(new Token("RecurringOrder.Date", recurringOrder.NextOrderUtc.ToString("dddd, dd MMMM yyyy")));
        tokens.Add(new Token("RecurringOrder.Id", recurringOrder.Id));
        tokens.Add(new Token("RecurringOrder.SubscriptionId", recurringOrder.SubscriptionId));


        StringBuilder sb = new StringBuilder();

        // shipping address
        sb.Append($"<p>{initialOrder.ShippingAddress.FirstName} {initialOrder.ShippingAddress.LastName}</p>");
        sb.Append($"<p>{initialOrder.ShippingAddress.Address1}</p>");
        if (!String.IsNullOrEmpty(initialOrder.ShippingAddress.Address2))
            sb.Append($"<p>{initialOrder.ShippingAddress.Address2}</p>");
        sb.Append($"<p>{initialOrder.ShippingAddress.City}, {initialOrder.ShippingAddress.StateProvince.Name} {initialOrder.ShippingAddress.ZipPostalCode}</p>");
        tokens.Add(new Token("RecurringOrder.ShippingAddress", sb.ToString(), true));

        // table of products
        sb.Clear();
        sb.Append("<table style='border:1px solid;'><tr style='border:1px solid;'>");
        sb.Append("<th style='border:1px solid;'>Sku</th>");
        sb.Append("<th style='border:1px solid;'>Name</th>");
        sb.Append("<th style='border:1px solid;'>Qty</th>");
        sb.Append("</tr>");
        foreach (var item in items)
        {
            sb.Append("<tr style='border:1px solid;'>");
            sb.Append($"<td style='border:1px solid;'>{item.Product.Sku}</td>");
            sb.Append($"<td style='border:1px solid;'>{item.Product.Name}</td>");
            sb.Append($"<td style='border:1px solid;'>{item.Quantity}</td>");
            sb.Append("</tr>");
        }
        sb.Append("</table>");
        tokens.Add(new Token("RecurringOrder.Items", sb.ToString(), true));
